...
public class PantallaInicio extends JFrame {
    ...
    public PantallaInicio() {
    	...
    	int alturaFrame = (int) tamanoFrame.getHeight();
    	int anchuraFrame = (int) tamanoFrame.getWidth();
        panel = new JPanel();
        panel.setLayout(null); 
        panel.setPreferredSize(new Dimension(anchuraFrame, alturaFrame));
        

        // Fondo de pantalla
        FondoPantalla pantallaInicio = new FondoPantalla("/Imagenes/pi.png");
        pantallaInicio.setBounds(0, 0, anchuraFrame, alturaFrame);
        // Texto de inicio
        JLabel textoInicio = new JLabel("Press Enter to Start",JLabel.CENTER);
        textoInicio.setFont(new Font("Arial", Font.PLAIN, 30));
        textoInicio.setSize(400, 100); // Establecemos un tamaño fijo para el JLabel
        textoInicio.setLocation(
            (anchuraFrame - textoInicio.getWidth()) / 2, // Centrado horizontalmente
            (alturaFrame - textoInicio.getHeight()) / 2 // Centrado verticalmente
        );
       // System.out.println(textoInicio.getWidth());

        // Agregar elementos al panel usando un JLayeredPane
        JLayeredPane layeredPane = new JLayeredPane();
        layeredPane.setBounds(0, 0, anchuraFrame, alturaFrame);
